package com.example.Assignment9.Services;
...
import com.example.Assignment9.Repository.TraineJpaRepository;
...
class EmployeeServiceTest {
	...
	void testFindTrineeById() {
		...
		Trainee actualObject = traineeService.findById(7);
	...
	void testFindTraineeByIdForNull() {
		...
		Trainee result = traineeService.findById(10);
	...
	void testFindByTraineeName() {
		...
		List<Trainee> result = traineeService.byName("aman");
	...
	void testInsertTrainee() {
		...
		Trainee result = traineeService.insert(trainee);
	...
	void testDeleteTraineeSuccess() {
		...
		boolean result = traineeService.deleteById(1);
	...
	void testFindTraineeByIdForException() {
		Mockito.when(traineeRepo.findById(10)).thenReturn(Optional.empty());
		assertThrows(RuntimeException.class, () -> traineeService.findById(10));
		Mockito.verify(traineeRepo, Mockito.times(1)).findById(10);
	}
...
//	@Disabled
	...
	void testFindTraineeByIdForNoException() {
		...
		assertDoesNotThrow(() -> traineeService.findById(7));
		Mockito.verify(traineeRepo, Mockito.times(1)).findById(7);
	}
}